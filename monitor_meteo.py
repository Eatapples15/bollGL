...
class SudAlertV13:
    ...
    def process(self):
        ...
        all_raw_codes = [] # Per il log di debug
        ...
        try:
            ...
            for item in items:
                ...
                # Estraiamo tutti i codici per capire cosa succede
                raw_c = str(p.get("Codice_Area", p.get("codice", ""))).upper()
                if raw_c: all_raw_codes.append(raw_c)
                
                crit = str(p.get("Idro", "VERDE")).upper()
                if crit in ["1", "VERDE", "0"]: crit = "VERDE"
                ...
                elif "BASI" in raw_c or "BAS-" in raw_code: # Gestione Basilicata
        ...
        print(f"DEBUG: Primi 10 codici trovati nel file: {all_raw_codes[:10]}")
        ...
        # Scrittura JSON con timestamp interno per forzare Git
        results["metadata"] = {
